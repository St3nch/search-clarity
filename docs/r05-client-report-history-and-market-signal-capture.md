...
> While doing paid SEO work, if we see a niche, product category, keyword cluster, or marketplace gap that looks promising, we log it as a raw signal and hand it off for later Neon Ronin scoring and research.

That is what this document defines. Client work reveals possible opportunities. SearchClarity logs the signal. Neon Ronin owns scoring, independent research, strategy routing, and later conversion into content, services, projects, or product/listing ideas.
...
## 5. Generalized Market Learning Handoff

SearchClarity does **not** own the generalized market learning model or the observatory.

SearchClarity's role is to capture useful raw material while doing paid report work:

- client/report history
- listing/page snapshots
- keyword observations
- recommendation pattern tags
- customer-shared outcomes, with confidence labels
- raw market opportunity signals
- analyst notes that may become generalized observations later

Neon Ronin owns the generalized learning layer. It decides how captured signals become patterns, scores, research queues, strategy decisions, content opportunities, service improvements, or future business ideas.

Clean handoff:

`	ext
Client-specific report work
    -> SearchClarity captures history and raw signals
    -> SearchClarity removes or avoids customer-identifying detail
    -> sanitized signal/export goes to Neon Ronin
    -> Neon Ronin scores, clusters, researches, and routes opportunities
`

The detailed generalized market learning examples, scoring logic, research workflow, observability metrics, and future platform data model are maintained in:

`	ext
C:\dev\neon-ronin\research-docs\r10-observatory-and-strategy-layer.md
`

SearchClarity keeps only the capture requirements needed to support paid customer work and safe internal handoff.
...
Paid report work
    ...
    → signal is handed off to Neon Ronin when appropriate
...
SearchClarity does not own opportunity scoring.

For paid report work, analysts may add a simple routing label so the signal is not lost:
...
The full scoring model belongs to Neon Ronin's observatory and strategy layer. SearchClarity should capture enough evidence for Neon Ronin to score the signal later, but analysts should not be forced into a heavy scoring rubric while fulfilling a paid customer report.
...
| routing_label | Lightweight SearchClarity label: Ignore / Watch / Research Later / Research Next / Urgent Review |
...
SearchClarity does not own the research workflow for strong signals.

SearchClarity's responsibility is to log the raw signal clearly enough that it can be researched later:

1. What was noticed.
2. Where it appeared in the paid work.
3. Why it seemed commercially or strategically interesting.
4. What evidence was visible at the time.
5. What a sensible next step might be.

Neon Ronin owns follow-up research, public-data validation, scoring updates, queue placement, and conversion into content, service improvements, internal projects, or product/listing ideas.
...
SearchClarity does not own the full market-opportunity review cadence.

SearchClarity should perform one lightweight post-report check:

- Did this paid report surface any raw market signals worth logging?
- If yes, add them to the tracker before closing the order.

Monthly or quarterly review of signals, scoring, research queues, and opportunity conversion belongs to Neon Ronin.
...
These labels can be exported to Neon Ronin as sanitized outcome inputs. Neon Ronin owns any generalized pattern-confidence calculations based on aggregated outcomes.

---

## 14. Observability / Intelligence Metrics Handoff

SearchClarity may track basic operational metrics needed to improve paid service delivery:
...
- repeat-order paths
- customer-shared outcomes, with confidence labels
- raw signals logged during fulfillment

Cross-business observability, intelligence metrics, strategy queues, signal scoring, opportunity validation, and pattern-confidence calculations belong to Neon Ronin.

SearchClarity should capture clean source data. Neon Ronin should turn that data into observatory-level intelligence.
## 15. Future Data Model Handoff

This document does not define the full future database model.

SearchClarity only defines the business records it needs to fulfill paid services and support returning customers:

- customers
- shops/sites/properties
- orders
- reports
- report files
- intake answers
- audited listings/pages
- keyword observations
- recommendations
- action plan items
...
The shared database architecture, workspace separation, observatory entities, generalized observation model, strategy queues, scoring tables, agent permissions, and cross-business storage rules belong to Neon Ronin.

When SearchClarity moves beyond spreadsheets, its data model should be implemented as a Neon Ronin business workspace, not as a separate strategy platform.
...
### Generalized market learning handoff
...
- Every signal gets logged with enough context for Neon Ronin to score later.
- Neon Ronin owns research queue placement, prioritization, and serious-opportunity classification.
...
6. A **raw market opportunity signals sheet** (signal ID, date, source engagement type, niche description, signal description, evidence summary, routing label, handoff status, notes). This is the tab that preserves paid-work signals for Neon Ronin handoff.
...
**End of r05 — Client Report History + Market Signal Capture.**
